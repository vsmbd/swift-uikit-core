//
//  UIView+Edge.swift
//  UIKitCore
//
//  Created by vsmbd on 04/02/26.
//

#if os(iOS) || os(tvOS)

import UIKit

// MARK: - UIView.Edge and EdgeVector

public extension UIView {
	enum Edge: Sendable {
		case lead
		case trail
		case top
		case bottom

		public static let all: [Self] = horizontalEdges + verticalEdges
		public static let verticalEdges: [Self] = [.top, .bottom]
		public static let horizontalEdges: [Self] = [.lead, .trail]
	}

	struct EdgeVector: Sendable {
		private(set) var edge: Edge
		private(set) var constraint: Constraint

		init(_ edge: Edge, _ constraint: Constraint = .equal) {
			self.edge = edge
			self.constraint = constraint
		}

		@inlinable
		public static var lead: Self { lead() }

		@inlinable
		public static var trail: Self { trail() }

		@inlinable
		public static var top: Self { top() }

		@inlinable
		public static var bottom: Self { bottom() }

		public static func lead(_ constraint: Constraint = .equal) -> Self {
			.init(.lead, constraint)
		}

		public static func trail(_ constraint: Constraint = .equal) -> Self {
			.init(.trail, constraint)
		}

		public static func top(_ constraint: Constraint = .equal) -> Self {
			.init(.top, constraint)
		}

		public static func bottom(_ constraint: Constraint = .equal) -> Self {
			.init(.bottom, constraint)
		}
	}

	struct EdgeConstraints: Sendable {
		public var lead: NSLayoutConstraint?
		public var trail: NSLayoutConstraint?
		public var top: NSLayoutConstraint?
		public var bottom: NSLayoutConstraint?

		@MainActor
		public var activate: Bool = false {
			didSet {
				lead?.isActive = activate
				trail?.isActive = activate
				top?.isActive = activate
				bottom?.isActive = activate
			}
		}

		mutating func set(_ edge: Edge, _ constraint: NSLayoutConstraint?) {
			switch edge {
			case .lead: lead = constraint
			case .trail: trail = constraint
			case .top: top = constraint
			case .bottom: bottom = constraint
			}
		}
	}
}

#endif
